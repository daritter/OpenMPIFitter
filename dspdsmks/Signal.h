...
namespace PAR {
    ...
    PARAM(sig_Mbc_sigma);
    PARAM(sig_Mbc_norm1);
    PARAM(sig_Mbc_meanshift1_m0);
    PARAM(sig_Mbc_meanshift1_m1);
    PARAM(sig_Mbc_sigmascale1);
    //PARAM(sig_Mbc_norm2);
    //PARAM(sig_Mbc_meanshift2);
    //PARAM(sig_Mbc_sigmascale2);
    ...
    PARAM(sig_dE_norm2);
    PARAM(sig_dE_meanshift2);
    PARAM(sig_dE_sigmascale2);
    PARAM(sig_dE_cheb1);
};
...
class Signal: public Component {
    ...
    virtual double operator()(const DspDsmKsEvent& e, const std::vector<double> &par) {
        ...
        signalPDF.fcn1.fcnx.set(
                ...
                par[PAR::sig_Mbc_meanshift1_m0] + e.dE*par[PAR::sig_Mbc_meanshift1_m1],
                par[PAR::sig_Mbc_sigmascale1]//, &par[PAR::sig_Mbc_norm2]
        );
    ...
    Add2DFcn<CompoundFcn2D<MultiGauss<2>, MultiGauss<3> >, CompoundFcn2D<Argus, Chebychev1> > signalPDF;
};
